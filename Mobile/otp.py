...
from accounts.send_mail import send_email
from accounts.serializers import ForgetPasswordSerializer
from Mobile.serializers import mobileEmailResetSerializer
from accounts.utils import check_email_exists
...
class  mobileVerifyEmailAndSendOTP(APIView):
    def post(self, request):
        input_data = request.data
        recipient = input_data.get("email",'')
        email_registered = check_email_exists(recipient)
...
class  mobileVerifyOTPAndChangePassword(APIView):
    """
...
class mobileVerifyAndChangeEmail(APIView):
    serializer_class=mobileEmailResetSerializer
    def post(self,request):
        input_data=request.data
        serializer=self.serializer_class(data=input_data)
        if serializer.is_valid():
